...
function chooseNumber() {
  let number;
  do {
    // Demande au joueur 1 d’entrer un nombre
    number = Number(prompt("Joueur 1 : Choisissez un nombre entre 0 et 50"));
    // La boucle continue tant que :
    // - Ce n’est pas un nombre (isNaN)
    // - Le nombre est inférieur à 0 ou supérieur à 50
  } while (isNaN(number) || number < 0 || number > 50);
  
  // Une fois un nombre valide donné, on le retourne
  return number;
}
